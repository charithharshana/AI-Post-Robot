...
class ImageEditorIntegration {
  ...
  async convertImageToDataUrl(imageUrl) {
    // Check if it's a Facebook or Pinterest URL which needs special handling
    ...
    if ((isFacebookUrl || isPinterestUrl) && typeof chrome !== 'undefined' && chrome.runtime) {
      ...
      console.log(`🔄 Using background script for ${platformName} image in editor...`);
      try {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => {
        if (isFacebookUrl) {
          ...
          reject(new Error('Pinterest image access blocked: Please save the image locally and upload manually'));
        } else {
