...
class RoboPostAPI {
  ...
  async uploadMediaFromUrl(imageUrl) {
    try {
      ...
      // Check if it's a Pinterest URL which also has CORS restrictions
      const isPinterestUrl = imageUrl.includes('pinimg.com') || imageUrl.includes('pinterest.com');
      if (isPinterestUrl) {
        console.log('📌 Detected Pinterest image URL - may have CORS restrictions');
      }
      ...
      try {
        ...
        try {
          ...
          // Add Pinterest-specific headers for no-cors mode
          if (isPinterestUrl) {
            noCorsHeaders['User-Agent'] = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
          }
          ...
          // Try background script for both Facebook and Pinterest URLs
          if ((isFacebookUrl || isPinterestUrl) && typeof chrome !== 'undefined' && chrome.runtime) {
            const platformName = isPinterestUrl ? 'Pinterest' : 'Facebook';
            console.log(`🔄 Trying background script fallback for ${platformName} image...`);
            try {
              ...
              if (isPinterestUrl) {
                throw new Error(`Pinterest image access blocked: All methods failed. Pinterest images are protected by CORS policies. Please try: 1) Save the image locally and upload manually, or 2) Use a different image source. Error: ${backgroundError.message}`);
              } else {
          ...
          } else if (isPinterestUrl) {
            throw new Error(`Pinterest image access blocked: Pinterest images often have CORS restrictions. Please try using a direct image URL or upload the image manually. Error: ${noCorsError.message}`);
          } else {
      ...
      // Check if it's a Facebook or Pinterest URL for specific error handling
      const isFacebookUrl = imageUrl.includes('facebook.com') || imageUrl.includes('fbcdn.net');
      const isPinterestUrl = imageUrl.includes('pinimg.com') || imageUrl.includes('pinterest.com');
      ...
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        if (isFacebookUrl) {
          ...
          throw new Error('Pinterest image access blocked: Pinterest images are protected by CORS policies and cannot be directly accessed. Please try: 1) Save the image locally and upload manually, or 2) Use a different image source.');
        } else {
      ...
      if (isPinterestUrl && (error.message.includes('CORS') || error.message.includes('blocked') || error.message.includes('empty'))) {
        throw new Error('Pinterest image access restricted: Pinterest protects their images with CORS policies. Please try: 1) Right-click the image and "Save image as..." then upload manually, or 2) Use a direct image URL from another source.');
      }
  ...
  async testImageUrl(imageUrl) {
    try {
      ...
      // If Facebook or Pinterest URL, try background script
      if ((isFacebookUrl || isPinterestUrl) && typeof chrome !== 'undefined' && chrome.runtime) {
        const platformName = isPinterestUrl ? 'Pinterest' : 'Facebook';
        try {
          const blob = await this.fetchImageViaBackground(imageUrl);
          ...
          return {
            ...
            error: `${platformName} URL not accessible: ${bgError.message}`,
